import { useState } from "react";
import { createFileRoute, Link } from "@tanstack/react-router";
import { useApp } from "@/context/AppContext";
import { ToneSelector } from "@/components/generator/ToneSelector";
import { LanguageSelector } from "@/components/generator/LanguageSelector";
import { generateThread } from "@/services/appService";
import type { ThreadResponse } from "@/types";

export const Route = createFileRoute("/thread")({
  head: () => ({ meta: [{ title: "Generator thread — Scripty" }] }),
  component: ThreadPage,
});

function CopyButton({ text }: { text: string }) {
  const [copied, setCopied] = useState(false);
  return (
    <button
      onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      className="rounded-full glass-pill px-2.5 py-1 text-xs font-medium text-primary hover:opacity-80 transition-opacity flex-shrink-0"
    >
      {copied ? "✓" : "Copiaza"}
    </button>
  );
}

function CopyAllThread({ posts }: { posts: string[] }) {
  const [copied, setCopied] = useState(false);
  return (
    <button
      onClick={() => { navigator.clipboard.writeText(posts.join("\n\n")); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      className="rounded-full px-3 py-1 text-xs font-medium text-muted-foreground border border-border hover:text-foreground transition-colors"
    >
      {copied ? "Thread copiat ✓" : "Copiaza thread complet"}
    </button>
  );
}

function SkeletonThread() {
  return (
    <div className="space-y-2 animate-pulse">
      {[1,2,3,4,5].map((i) => (
        <div key={i} className="glass rounded-2xl p-4 space-y-2">
          <div className="skeleton h-4 w-full rounded-full" />
          <div className="skeleton h-4 w-4/5 rounded-full" />
        </div>
      ))}
    </div>
  );
}

function ThreadPage() {
  const { tone, language, addToHistory } = useApp();
  const [text, setText] = useState("");
  const [maxPosts, setMaxPosts] = useState(8);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<ThreadResponse | null>(null);

  const handleGenerate = async () => {
    setError(null);
    if (!text.trim()) { setError("Introdu un text sau subiect pentru thread."); return; }
    setLoading(true);
    try {
      const r = await generateThread({ text, max_posts: maxPosts, language, tone });
      setResult(r);
      addToHistory({ type: "thread", preview: text.slice(0, 60), result: r });
    } catch (e) {
      setError(e instanceof Error ? e.message : "Eroare necunoscuta");
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="mx-auto max-w-2xl px-6 py-12 pb-24">
      <div className="mb-8 flex items-center gap-3">
        <Link to="/" className="text-sm text-muted-foreground hover:text-foreground transition-colors">← Acasa</Link>
        <span className="text-border">·</span>
        <h1 className="text-lg font-semibold text-foreground">Generator thread X</h1>
      </div>

      <div className="glass rounded-2xl p-6 space-y-5">
        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
            Text sau subiect
          </label>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Lipeste un articol, un blog post sau descrie subiectul pe care vrei sa il transformi intr-un thread pentru X (Twitter)..."
            rows={6}
            className="glass-sm rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/60 resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 w-full"
          />
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                Postari in thread
              </label>
              <span className="text-xs font-bold text-primary tabular-nums">{maxPosts}</span>
            </div>
            <input
              type="range" min={3} max={15} value={maxPosts}
              onChange={(e) => setMaxPosts(Number(e.target.value))}
              className="w-full h-1.5 rounded-full appearance-none cursor-pointer"
              style={{ accentColor: "oklch(0.52 0.24 295)" }}
            />
          </div>
          <ToneSelector />
        </div>

        <LanguageSelector />

        <button
          onClick={handleGenerate}
          disabled={loading}
          className="w-full rounded-xl bg-accent py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity disabled:opacity-40"
        >
          {loading ? "Se genereaza..." : "Genereaza thread"}
        </button>
      </div>

      {error && (
        <div className="mt-4 rounded-2xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
          {error}
        </div>
      )}

      {loading && <div className="mt-6"><SkeletonThread /></div>}

      {result && !loading && (
        <div className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
              Thread · {result.thread.length} postari
            </p>
            <CopyAllThread posts={result.thread} />
          </div>
          <div className="space-y-2">
            {result.thread.map((post, i) => (
              <div key={i} className="glass rounded-2xl p-4 flex items-start gap-3">
                <span className="text-xs font-bold text-primary/40 tabular-nums mt-0.5 flex-shrink-0 w-5">
                  {i + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground leading-relaxed">{post}</p>
                  <p className="text-[10px] text-muted-foreground mt-1.5 tabular-nums">
                    {post.length}/280 caractere
                  </p>
                </div>
                <CopyButton text={post} />
              </div>
            ))}
          </div>
        </div>
      )}
    </div>
  );
}
